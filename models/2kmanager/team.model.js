...
class Team extends Connection {
    ...
    getSingleTeamWithMembers(params) { 
        const sql = 'SELECT wt.*, w.name, w.image_name FROM wrestler_team wt INNER JOIN wrestler w ON w.id = wt.wrestler_id WHERE wt.team_id = ' + params.id;
        return this.query(sql);
    }
    ...
    createTeam(body) {
        ...
        const missingEntries = Object.entries(body).filter(
            ([key, _]) => {
